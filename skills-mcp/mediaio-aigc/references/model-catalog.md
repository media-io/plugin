...
>
> Transitional note: this MCP variant is currently derived from the CLI catalog. `capability_code` here is the same identifier the CLI calls `capability_code` and the upstream registry calls `uni_fun_code`. The generator will emit both variants directly (see the skill design doc, S8).
...
| Scenario | Default model | capability_code | Fallback | capability_code |
| --- | --- | --- | --- | --- |
...
| Display name | capability_code | Access tier* | Inputs | When to pick it |
| --- | --- | --- | --- | --- |
...
| Seedance 2.5 | `image2video_seedance _2.5` | unknown | image, prompt | Long video, audio included, cinematic quality. The capability_code contains one space; that is the real production value. |
...
\* Access tier is **manually curated**; the registry has no such field. `unknown` means it has not been confirmed with the product team. Whether a job actually costs credits is decided by `estimate_generation` and the server response — never tell the user a model is free based on this file.
...
| # | Condition | Pick | capability_code |
| --- | --- | --- | --- |
...
| 3 | Everything else, including character consistency, outfit swap and character editing, AND `get_account` reports `membership.is_member: true` | ToMoviee 3.0 Pro | `image2image_media_3.0` |
...
Before falling back, confirm the target is still online under the discovery guardrail in `SKILL.md`, then repeat the credit confirmation step.
...
| capability_code | Display name | Description |
| --- | --- | --- |
...
### 6.1 capability_code contains a space (6)

These are the real production values and **will not be changed**. Writing them without the space fails with `CAPABILITY_NOT_FOUND`. Pass the value exactly as written, space included — there is no shell on this path, so it is never quoted or escaped.

| capability_code | Display name |
| --- | --- |
...
Never infer a model's module from its capability_code prefix. Use this table.

| capability_code | Actual fun_module |
| --- | --- |
...
| Display name | capability_code candidates |
| --- | --- |
...
### 6.4 Display name and capability_code disagree

A historical artifact; the upstream `uni_fun_code` cannot be changed. **Map display name to capability_code only, never the reverse.** Typical examples:

| capability_code | Display name | Commonly misread as |
| --- | --- | --- |
...
| capability_code | Display name | Module |
| --- | --- | --- |
...
## 7. Live-lookup reference

Call `list_capabilities` only under a trigger listed in the skill's discovery guardrail. `module` is the only filter the server actually applies, and it must be one of the exact values enumerated in that tool's description — an unrecognised value is not an error, it just returns nothing useful.

After every selection, call `describe_capability` for the parameter schema. This file makes no promise about parameters and must not be used to infer them.

> `list_capabilities` sees more rows than this file documents, because the catalog is a curated subset of the registry. Absence from this file is not proof that a capability does not exist.
